export const homePage = {
  contentLabel: 'Home',
  title: 'waggon | Your local online farmers market',
  description: 'An online platform for local vendors to sell their products online. A simple set of tools designed to quickly get your products online and available.'
}

export const contactPage = {
  contentLabel: 'Contact',
  title: 'waggon | Contact',
  description: 'Get a hold of a waggon professional today.'
}

export const vendorsPage = {
  contentLabel: 'Vendors',
  title: 'waggon | Vendors',
  description: 'Sell your products to customers in your area.'
}

export const pricingPage = {
  contentLabel: 'Pricing',
  title: 'waggon | Pricing',
  description: 'Create your online store in minutes and start selling your products.'
}

export const supportPage = {
  contentLabel: 'Support',
  title: 'waggon | Support',
  description: 'Get the help you need to keep your business moving forward.'
}

export const termsPage = {
  contentLabel: 'Terms',
  title: 'waggon | Terms',
  description: 'Get all the legal facts about waggon and the services we provide.'
}

export const privacyPage = {
  contentLabel: 'Privacy',
  title: 'waggon | Privacy',
  description: 'Find out how we are keeping our customers data safe and secure.'
}
